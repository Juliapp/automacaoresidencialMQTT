...
const Jardim = ({ setOpen }) => {
  const { client, jardimLuzMax, jardimLuzMin } = useMqtt();

  const cancelButtonRef = useRef(null);

  const [minValue, setMinValue] = useState(
    `${jardimLuzMin < 10 ? '0' + jardimLuzMin : jardimLuzMin}:00`
  );
  const [maxValue, setMaxValue] = useState(
    `${jardimLuzMax < 10 ? '0' + jardimLuzMax : jardimLuzMax}:00`
  );
  ...
  return (
    <div className="bg-gray-200 dark:bg-gray-900">
      <span className="flex justify-center dark:text-gray-50">Jardim</span>
      <label className="block text-gray-500 text-sm font-bold mb-2">
        A iluminação do jardim funciona em um intervalo de tempo durante o dia.
      </label>
      <label className="block text-gray-500 text-sm font-bold mb-2">
        Horário Mínimo
      </label>
